...
// List lists all HttpFilters in the indexer.
func (s *httpFilterLister) List(selector labels.Selector) (ret []*v1.HttpFilter, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.HttpFilter))
	})
	return ret, err
}
...
// List lists all HttpFilters in the indexer for a given namespace.
func (s httpFilterNamespaceLister) List(selector labels.Selector) (ret []*v1.HttpFilter, err error) {
...
// Get retrieves the HttpFilter from the indexer for a given namespace and name.
func (s httpFilterNamespaceLister) Get(name string) (*v1.HttpFilter, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("httpfilter"), name)
	}
	return obj.(*v1.HttpFilter), nil
}
